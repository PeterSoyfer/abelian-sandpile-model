...
{   //periodic boundary condition - at the end of the table files are thrown away
...
std::vector<std::vector<int>> init_toppling_matrix_inf()
{   //no boundary conditions - round table, NO files are thrown away!
	std::vector<std::vector<int>> toppling_matrix_inf(N, std::vector<int>(N));
	...
	{		
		for (int j = 0; j < N; ++j)
		{
			...
			{
				toppling_matrix_inf[i][j] = -1;

				if (N == 2)
					toppling_matrix_inf[i][j] = -2;
			}
	...
	//print_matrix(toppling_matrix_inf);
	return toppling_matrix_inf;
}
...
std::vector<int> stabilise(const std::vector<int>& v, std::vector<int>& v_topplings, const std::vector<std::vector<int>>& toppl_mx)
{
	...
	while ( (*max_element(v_stable.begin(), v_stable.end()) > 2) && (iterator <= 30) )
	{
		for (int i = 0; i < N; ++i)
		{
			if (v_stable[i] > 2)
			{			
				//++topplings_vector[i];
				std::transform(v_stable.begin(), v_stable.end(), toppl_mx[i].begin(), v_stable.begin(), std::minus<int>());
				++iterator;
				
				print_vector(v_stable, max_width(v_stable));
				std::cout << std::endl;
			}
		}
	}
...
void all_configs_total_height(const int sum_heights, const int max_height)
{ //printing all the configurations with given total sum of heights and maximal height for each site
	std::vector<int> config(N);
	int all_configs_n = 0;
	//print_vector(config, max_width(config));
	//std::cout << std::endl;
	for (
}
...
int main()
{
	...
	//delta = init_toppling_matrix();
	//print_matrix(delta);
	delta = init_toppling_matrix_inf();
	print_matrix(delta);
	...
	all_configs_total_height(0, 0);
	//u = stabilise(v, w, delta);
